...
from pyqt_ext import AbstractTreeView, ColorButton, toQColor, toColorStr, KeyValueTreeItem, KeyValueTreeModel, KeyValueTreeView
import pyqtgraph as pg
from pyqtgraph_ext import AxisRegion, XAxisRegion, XAxisRegionTreeItem, XAxisRegionTreeModel


class XAxisRegionTreeView(AbstractTreeView):
    ...
    def setModel(self, model: XAxisRegionTreeModel):
        AbstractTreeView.setModel(self, model)
    ...
    def selectionChanged(self, selected: QItemSelection, deselected: QItemSelection):
        ...
        selection_items: list[XAxisRegionTreeItem] = [self.model().itemFromIndex(index) for index in self.selectionModel().selectedIndexes()]
    ...
    def editItem(self, item: XAxisRegionTreeItem):
    ...
    def selectedRegions(self) -> list[dict]:
        selectedItems = self.selectedItems()
        return [item._region for item in selectedItems if item.is_region()]
    ...
    def updateRegion(self, region: dict):
        ...
        for item in self.model().root().depth_first():
            if item.is_region() and item._region is region:
...
def test_live():
    ...
    from pyqtgraph_ext import Figure
    figs = []
    for i in range(2):
        fig = Figure()
        fig.plotItem._xdim = 'x'
        fig.show()
        figs.append(fig)
    plots = [fig.plotItem for fig in figs]
    view.setPlots(plots)
